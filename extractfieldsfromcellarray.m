function output = extractfieldsfromcellarray(database,fieldname)
...
if isempty(database)
    ...
    for i = 1:length(fieldname)
        ...
        if not(isfield(database{1}, thisfieldname))
            ...
            if ischar(database{1}.(thisfieldname)) || isstruct(database{1}.(thisfieldname)) || iscell(database{1}.(thisfieldname))
