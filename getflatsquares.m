...
%    f1cheat = makecheats(f1,kit,linetouse,ts);
    fprintf('%s: jumping in, rank %d, height %3.1f f = %3.1f\n',kit.molname,rank,h1,f1);
%     newhs = [];
%     
%     f = 6;
%     hthresh = 1e10;
                    ...
                    for mm = 1:3
                     ...
                     %       [minval ival] = closest(f4,fs);
                            ...
                            if minval < ts.flatsquarefthresh
                                ...
                                s.frange = frange;
                                if laddermode == 1
                                    if isfield(kit,'visiblewindow')
                                        s.frecommendedmin = kit.visiblewindow(1);
                                        s.frecommendedmax = kit.visiblewindow(2);
                                    else
...
for i = 1:length(allsquares)
    ...
    for k = 1:length(newsquares)
        
        thatsquare = newsquares{k};
        sortfs = sort(thissquare.fs(1:2));
        sortfs2 = sort(thatsquare.fs(1:2));
        maxdiff12 = max(abs(sortfs - sortfs2));
        sortfs = sort(thissquare.fs(3:4));
        sortfs2 = sort(thatsquare.fs(3:4));
        maxdiff34 = max(abs(sortfs - sortfs2));
        if (maxdiff12 < 0.1) && (maxdiff34 < 0.1)
            seemsnew = 0;
        end
    end
    if seemsnew
        mindiff = min(diff(sort(thissquare.fs)));
        if mindiff > 1
            newsquares{end+1} = thissquare;
        end
    end
end
